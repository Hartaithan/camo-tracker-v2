			...
			<div className="tracker_sidebar_container_collapse">
				{items.map((item, index) => (
					<div key={props.mast + item.name}>
						<Collapsible handleTriggerClick={() => dispatch({ type: "TOGGLE_COLLAPSIBLE", mast: props.mast, id: index })} open={collapsibleId[index] ? true : false} trigger={collapsibleId[index] ? arrow(true, item.name) : arrow(false, item.name)} transitionTime={100}>
