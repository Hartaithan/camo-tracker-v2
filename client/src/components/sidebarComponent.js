...
function Sidebar() {
  ...
  function CollapsibleList(props) {
    const collapsibleId =
      props.mast === "dm" ? app.collapsibleIsOpen.dm : app.collapsibleIsOpen.da;
        ...
        {items.map((item, index) => (
          <div key={props.mast + item.name}>
            <Collapsible
              ...
              trigger={
                collapsibleId[index]
                  ? arrow(true, item.name)
                  : arrow(false, item.name)
              }
              transitionTime={100}
            >
              {item.weapons.map((weapon) => (
                <div
                  className="tracker_sidebar_container_collapse_listitem"
                  key={props.mast + weapon.name}
                >
  ...
  return (
    ...
    >
      ...
      <div className="tracker_sidebar_tabview">
        <div
          className="tracker_sidebar_tabview_tab"
          onClick={() => dispatch({ type: "TOGGLE_TAB", mast: "dm" })}
        >
          ...
          <div
            className="tracker_sidebar_tabview_tab_line"
            style={{ opacity: app.isActive === "dm" ? "1" : "0" }}
          ></div>
        </div>
        <div
          className="tracker_sidebar_tabview_tab"
          onClick={() => dispatch({ type: "TOGGLE_TAB", mast: "da" })}
        >
          <div className="tracker_sidebar_tabview_tab_title">
          ...
          <div
            className="tracker_sidebar_tabview_tab_line"
            style={{ opacity: app.isActive === "da" ? "1" : "0" }}
          ></div>
        </div>
      </div>
      <div className="tracker_sidebar_container">
        {app.isActive === "dm" ? (
          <CollapsibleList mast="dm" />
        ) : (
          <CollapsibleList mast="da" />
        )}
      </div>
    </div>
  );
