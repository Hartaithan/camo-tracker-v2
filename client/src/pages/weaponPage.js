		...
		<div className="tracker_container" style={app.isOpen ? style.isOpen : style.isClosed}>
			...
			<div className="tracker_weapon">
				<div className="tracker_weapon_wrapper">
					{camos.map((item, i) => (
						<div className="tracker_weapon_wrapper_container" onMouseEnter={() => setShowCheck(i)} onMouseLeave={() => setShowCheck(i)} key={item.id}>
							...
							<div className="tracker_weapon_wrapper_container_camocontainer">
								{item.camos.map((camo, index) => {
									if (camo.id === 36 || camo.id === 37 || camo.id === 38) {
										...
										return (
											<div className="tracker_weapon_wrapper_container_camocontainer_camo" onClick={() => setCamo(camo.id, index, i)} key={camo.id} data-title={getMission(item.id, index)}>
												<div
