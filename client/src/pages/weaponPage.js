...
function WeaponPage() {
  ...
  function getMission(id_cat, id_weap, id_camo_cat, id_camo) {
    const [first, second] =
      mission[id_cat - 1].weapons[id_weap - 1].camos[
        id_camo_cat - 1
      ].mission.split("_");
    const goal =
  ...
  function getMasteryMission(id_cat, id_weap, id_camo_cat, id_camo) {
    const message =
      mission[id_cat - 1].weapons[id_weap - 1].camos[id_camo_cat - 1].mission[
        id_camo
      ].split("_");
    return message;
  }
        ...
        <div className="tracker_weapon_wrapper">
          {camos.map((item, index) => (
            <div
              ...
              onMouseEnter={() => setShowCheck(index)}
              onMouseLeave={() => setShowCheck(index)}
              key={item.id}
            >
