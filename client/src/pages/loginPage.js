	...
	const onSubmit = async (data) => {
		...
		try {
			await axios
				...
				.then((response) => {
					dispatch({ type: "LOG_IN", token: response.data.token, userId: response.data.userId, nick: response.data.nick });
					history.push("/");
					setLoading(false);
				})
	...
	return (
		<div className="tracker_container" style={{ marginLeft: app.isOpen ? "300px" : "0px" }}>
			...
			<div className="tracker_login">
				<form className="tracker_login_form" onSubmit={handleSubmit(onSubmit, onErrors)}>
					<div className="tracker_login_form_field">
						<label htmlFor="email">
							<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 455.862 455.862">
								<path
			...
			c5.43,6.257,4.76,15.731-1.497,21.16L301.162,227.93L403.394,316.659z"
					...
					<div className="tracker_login_form_submit">
						<input type="submit" value={loading ? "LOADING..." : "LOG IN"} disabled={loading} style={{ background: loading ? "#575757" : "#ffc400" }} />
					</div>
				</form>
